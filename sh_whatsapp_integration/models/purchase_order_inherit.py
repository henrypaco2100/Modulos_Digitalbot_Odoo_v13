        ...
        if self:
            for rec in self:
                ...
                if rec.company_id.purchase_order_information_in_message and rec.partner_id and rec.currency_id and rec.company_id:
                    txt_message +=  "Estimado/a: " + str(rec.partner_id.name)+","+"%0A%0A"+"Aquí está el pedido "+'*'+rec.name+'*'+" que asciende en "+'*'+str(rec.amount_total)+str(rec.currency_id.symbol)+'*'+" de "+rec.company_id.name+"%0A%0A"
                if rec.company_id.purchase_product_detail_in_message:
                    ...
                    if rec.order_line:
                        for purchase_line in rec.order_line:
                            txt_message +=  "%0A"+"*"+purchase_line.product_id.name+"*"+"%0A"+"*Cantidad:* "+str(purchase_line.product_qty)+"%0A"+"*Precio:* "+str(purchase_line.price_unit)+""+str(purchase_line.order_id.currency_id.symbol)+"%0A"+"________________________"+"%0A"
        ...
        if self:
            for rec in self:
                ...
                if rec.partner_id.mobile:
                    ...
                    raise UserError("El número de móvil del socio no existe")
