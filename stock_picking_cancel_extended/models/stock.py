            ...
            if move.picking_id.state == 'done' or 'confirmed':
                ...
                for pack_op_id in pack_op:
                        ...
                        for move_id in pack_op:
                            for line in move_id.move_line_ids:
                                ...
                                if line.lot_id:
                                    ...
                                    if lot_outgoing_quant.product_id.tracking in ['lot','serial'] or lot_source_quant.product_id.tracking in ['lot','serial']:
                                        if lot_outgoing_quant:
                                            for lot in lot_outgoing_quant:

                                                old_qty = lot.quantity
                                                lot.quantity = old_qty - line.qty_done
                                        else:
                                            vals = {'product_id': move.product_id.id,
                                                    'location_id': move.location_dest_id.id,
                                                    'in_date': move_id.date,
                                    ...
                                    if pack_op_id.location_dest_id.usage == 'customer':
                                        ...
                                        if outgoing_customer_quant:
                                            ...
                                            outgoing_customer_quant[0].quantity = old_qty - line.qty_done
                                        else:
                                            vals = {'product_id': move_id.product_id.id,
                                                    'location_id': move_id.location_dest_id.id,
                                                    'in_date': move_id.date,
                    ...
                    if move.picking_id.picking_type_id.code == 'incoming':
                        # Cancelar valoracion de entrada
                        for stock_valuation_layer in move.stock_valuation_layer_ids:
                                ...
                                if line.lot_id:
                                    if line.product_id.tracking in ['lot','serial']:
                                        ...
                                        if incoming_quant:
                                            ...
                                            incoming_quant[0].quantity = old_qty - line.qty_done
                                        else:
                                            vals = {'product_id': move_id.product_id.id,
                                                    'location_id': move_id.location_dest_id.id,
                                                    'in_date': move_id.date,
                                        ...
                                        if incoming_customer_quant:
                                            ...
                                            incoming_customer_quant[0].quantity = old_qty + line.qty_done
                                        else:
                                            vals = {'product_id': move_id.product_id.id,
                                                    'location_id': move_id.location_id.id,
                                                    'in_date': move_id.date,
...
class stock_quant(models.Model):
    ...
    def _update_reserved_quantity(self, product_id, location_id, quantity, lot_id=None, package_id=None, owner_id=None, strict=False):
        ...
        #print("producto uom", product_id.uom_id)
        rounding = product_id.uom_id.rounding
        #print("rouding",rounding)
        ...
        if float_compare(quantity, 0, precision_rounding=rounding) > 0:
            ...
            #solucion condicion de si reserva es igual a cero o no
            available_quantity = sum(quants.mapped('reserved_quantity'))
            if float_compare(abs(quantity), available_quantity, precision_rounding=rounding) > 0:
                # cuando entra por aca supone que la entrega fue realizada
                quants.reserved_quantity += abs(quantity)
