...
class gen_purchase(models.TransientModel):
    ...
    sequence_opt = fields.Selection([('custom', 'Usar Excel/CSV Numero de secuencia'), ('system', 'Usar el número de secuencia predeterminado del sistema')], string='Opciones de Secuencia',default='custom')
    import_option = fields.Selection([ ('xls', 'XLS Archivo')], string='Seleccionar', default='xls') # ('csv', 'CSV Archivo'),
    ...
    import_prod_option = fields.Selection([('name', 'Nombre'),('code', 'Codigo'),('barcode', 'Codigo de Barras')],string='Importar Productos por',default='name')
    ...
    def make_purchase(self, values):
        ...
        if pur_search:
            ...
            pur_id = purchase_obj.create({
                ...
                'picking_type_id':  values.get('warehouse_id'),
            })
    ...
    def make_purchase_line(self, values, pur_id):
        ...
        print(values['product'], 'producto name')
    ...
    def proceso_de_trabajo(self, name_proceso_flujo):
        objeto_proceso_de_trabajo = self.env['automated.purchase'].search([('name', '=', name_proceso_flujo)])
    ...
    def obtener_id_entregar_a(self, id_proceso_de_flujo):
        objeto_proceso_de_trabajo = self.env['automated.purchase'].search([('id', '=', id_proceso_de_flujo)])
        if objeto_proceso_de_trabajo:
            if objeto_proceso_de_trabajo.st_entregar_a:
                return objeto_proceso_de_trabajo.st_entregar_a.id
            else:
                raise Warning(
                    _('El Campo "Entregar a" "%s"  no existe en el proceso de flujo.') % objeto_proceso_de_trabajo.st_entregar_a.name)
        else:
            raise Warning(_('Proceso de flujo "%s" inválido.') % objeto_proceso_de_trabajo.name)
    def import_csv(self):
            ...
            for row_no in range(sheet.nrows):
                ...
                if row_no <= 0:
                    ...
                    values.update({'purchase_no':line[0],
                                   ...
                                   'work_process_order_id':self.proceso_de_trabajo(line[10]),
                                   'warehouse_id': self.obtener_id_entregar_a(self.proceso_de_trabajo(line[10])),
