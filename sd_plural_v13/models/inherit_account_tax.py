...
class SdInheritAccountTaxP(models.Model):
    ...
    def validaciones_pro_and_reg(self,product=None,tax=None,tipo=None):
        if not tipo:
            if product.sd_amount_impuesto <= 0:
                raise UserError(
                    _('El producto %s no cuenta con un importe predeterminado para el calculo de los impuesto Pro y Reg.'
                      '\n   -   producto -> contabilidad -> Importe: el monto debe ser mayor a 0')%(product.name))
            if product.sd_tipo_importe != tax.amount_type:
                raise UserError(
                    _('El producto %s no cuenta con el mismo Calculo de impuesto que el Impuesto %s'
                      '\n   -   producto -> contabilidad -> Cálculo de impuestos')%(product.name,tax.name))
        if tipo:
            tipe_dic = {
                'pro':'PRO',
                'reg': 'REG',
            ...
            if tipo in ['pro', 'reg']:
                if not product.sd_cuenta_pro or not product.sd_cuenta_reg:
                    raise UserError(
                        _('El producto %s no tiene la cuenta %s establecidas'
                          '\n   -   producto -> contabilidad -> PRO Y REG')%(product.name,tipe_dic[tipo]))
            elif tipo in ['coe_pro', 'coe_reg']:
                if not product.sd_cuenta_coe or not product.sd_cuenta_coe_reg:
                    raise UserError(
                        _('El producto %s no tiene la cuenta %s establecidas'
                          '\n   -   producto -> contabilidad -> COE PRO Y COE REG')%(product.name,tipe_dic[tipo]))
    def return_sd_amount_impuesto(self,sd_amount_impuesto,tax):
        if tax.sd_tipo_pro_reg in ['pro','coe_pro']:
            sd_amount_impuesto = 1* (sd_amount_impuesto)
        elif tax.sd_tipo_pro_reg in ['reg','coe_reg']:
            sd_amount_impuesto = -1 * (sd_amount_impuesto)
        else:
            raise UserError(
                _('El Impuesto %s no cuenta con un tipo de impuesto Pro o Reg'
                  '\n   -   producto -> contabilidad -> Cálculo de impuestos')%(tax.name))

        return sd_amount_impuesto
    def write_cuenta_pro_and_reg_in_taxes(self,tax_rep_lines,product):
        for tax_rep_line in tax_rep_lines:
            if tax_rep_line.tax_id.sd_tipo_pro_reg == 'pro':
                self.validaciones_pro_and_reg(product=product,tipo='pro')
                tax_rep_line.sudo().write({
                    'account_id': product.sd_cuenta_pro.id
                })
            elif tax_rep_line.tax_id.sd_tipo_pro_reg == 'reg':
                self.validaciones_pro_and_reg(product=product,tipo='reg')
                tax_rep_line.sudo().write({
                    'account_id': product.sd_cuenta_reg.id
                })
            elif tax_rep_line.tax_id.sd_tipo_pro_reg == 'coe_pro':
                self.validaciones_pro_and_reg(product=product,tipo='coe_pro')
                tax_rep_line.sudo().write({
                    'account_id': product.sd_cuenta_coe.id
                })
            elif tax_rep_line.tax_id.sd_tipo_pro_reg == 'coe_reg':
                self.validaciones_pro_and_reg(product=product,tipo='coe_reg')
                tax_rep_line.sudo().write({
                    'account_id': product.sd_cuenta_coe_reg.id
                })
    def add_automated_taxes_pro_and_reg(self,product,taxes):
        if product:
            for tax in taxes.filtered(lambda x: x.sd_tipo_pro_reg):
                self.validaciones_pro_and_reg(product=product,tax=tax)
                sd_amount_impuesto = self.return_sd_amount_impuesto(product.sd_amount_impuesto,tax)
                tax.sudo().write({
                    'amount': sd_amount_impuesto
                })
                tax_rep_lines = taxes.invoice_repartition_line_ids._origin.filtered(lambda x: x.repartition_type == "tax") + taxes.refund_repartition_line_ids._origin.filtered(lambda x: x.repartition_type == "tax")
                self.write_cuenta_pro_and_reg_in_taxes(tax_rep_lines,product)
    ...
    def compute_all(self, price_unit, currency=None, quantity=1.0, product=None, partner=None, is_refund=False,
        ...
        # ADICCIONAR IMPUESTOS PRO Y REG --HENRY
        self.add_automated_taxes_pro_and_reg(product, taxes)
            ...
            for repartition_line, line_amount in zip(tax_repartition_lines, repartition_line_amounts):
                ...
                taxes_vals.append({
                    ...
                    'account_id': tax.cash_basis_transition_account_id.id if tax.tax_exigibility == 'on_payment' \
                                                                             and not self._context.get(
                        'caba_no_transition_account') \
