...
class WizardReportOpenInvoice(models.TransientModel):
    _name = "wizard.report.open.invoice"
    _description = "Get Report Open Invoice"
    ...
    def generate_report(self):
        ...
        #data = []
        #for rec in obj_invoices:
        #    data.append(rec.id)
        #'invoice_ids': [(6, False, data)],
        rp_payment_sale_id = self.env['rp.open.invoice.report'].create({
        ...
        if self._context.get('excel_report'):
            ...
            #gr_doc_ids = self.env['account.move'].read_group(
            #    [('state', '=', 'posted'), ('state', '!=', 'draft'), ('move_type', '=', 'out_invoice'),
            #     ('payment_state', '!=', 'paid'), ('payment_state', '!=', 'reversed')],
            #    fields=['journal_id','partner_id', 'user_id', 'amount_total','amount_paid_tmp','amount_residual'],
            #    groupby=['journal_id', 'partner_id'])
            #
            # self.ensure_one()
            # docids = self.env['account.move'].search([('state', '=', 'posted'), ('state', '!=', 'draft'), ('move_type', '=', 'out_invoice'), ('payment_state', '!=', 'paid'), ('payment_state', '!=', 'reversed')]).ids
            # [data] = self.read()
            # data['move_ids'] = self.env.context.get('active_ids', [docids])
            # moves = self.env['account.move'].browse(data['move_ids'])
            # datas = {
            #     'ids': [],
            #     'model': 'account.move',
            #     'form': data
            # }
            #for rec in gr_doc_ids:
            #    a = 123
            #    b = 1234
            return self.env.ref('btc_route_sale.invoice_open_report_btc').report_action(rp_payment_sale_id)


class RpPaymentSaleBleichner(models.TransientModel):
    _name = "rp.open.invoice.report"
    _description = 'Open Invoice Report'
    _order = 'name desc,date desc'

    invoice_ids = fields.Many2many('account.move', 'move_id')
    name = fields.Char()
