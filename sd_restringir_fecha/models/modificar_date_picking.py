...
import datetime
import pytz
class InheritPickingDate(models.Model):
    _inherit = "stock.picking"

    def SobreEscribirFechaTransferencia(self):
        date_order = self.date_done or self.scheduled_date
        user_tz = self.env.user.tz
        local = pytz.timezone(user_tz)
        date_invoice = (pytz.utc.localize(date_order).astimezone(local)).date()
        objeto_stock_move = self.env['stock.move'].search([('picking_id', '=', self.id)])
            ...
            for stock_move in objeto_stock_move:
                ...
                if objeto_account_move:
                    for account_move in objeto_account_move:
                        account_move.write({'date': date_invoice})
                ...
                if objeto_stock_valuation_layer:
                    for stock_valuation_layer in objeto_stock_valuation_layer:
                        ...
                        self.env.cr.execute("UPDATE public.stock_valuation_layer SET create_date=%s WHERE id=%s ",
                                            (parametros))

        objeto_stock_move_line = self.env['stock.move.line'].search([('picking_id', '=', self.id)])
    ...
    def action_done(self):
        ...
        # como el campo fecha efectiva no esta cargado por defecto preguntamos si este tiene fecha
        # antes de escribir
        if self.date_done or self.scheduled_date:
            self.write({'date_done': self.date_done or self.scheduled_date})
        else:
            self.write({'date_done': fields.Datetime.now()})
        self.SobreEscribirFechaTransferencia()
